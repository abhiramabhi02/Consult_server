  ...
  try {
    ...
    if (allProfessionals) {
      ...
      res.send({
        status: 500,
        success: false,
        message: "professionals fetching failed",
      });
    }
  } catch (error) {
    res.send({ status: 500, success: false, message: error.message });
  ...
  try {
    ...
    if (update) {
      ...
      res.send({
        status: 500,
        success: false,
        message: "user status failed to updated",
      });
    }
  } catch (error) {
    res.send({ status: 500, success: false, message: error.message });
  ...
  try {
    ...
    if (update) {
      res.send({
        status: 200,
        success: true,
        message: "professional status updated",
      });
    } else {
      res.send({
        status: 500,
        success: false,
        message: "professional status failed to updated",
      });
    }
  } catch (error) {
    res.send({ status: 500, success: false, message: error.message });
...
const getAppointments = async (req, res) => {
  try {
    const appointments = await appointentcontrol.getAllAppointments(
      Appointment
    );
    ...
    if (appointments) {
      ...
      res.send({
        status: 500,
        success: false,
        message: "error fetching in appointments",
      });
    }
  } catch (error) {
    res.send({ status: 500, success: false, message: error.message });
...
const dashboardData = async (req, res) => {
  try {
    ...
    if (users && professionals && appointements) {
      const userCount = users.length;
      const proCount = professionals.length;
      const appointmentCount = appointements.length;
      res.send({
        status: 200,
        user: userCount,
        pro: proCount,
        appointment: appointmentCount,
        appointmentdata: appointements,
        success: true,
        message: "Success",
      });
    } else {
      res.send({ status: 500, success: false, message: "Failure" });
    }
  } catch (error) {
    res.send({ status: 500, success: false, message: error.message });
  }
